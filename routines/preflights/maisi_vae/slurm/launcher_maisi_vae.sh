#!/usr/bin/env bash
# Submit the BraTS-2026 MAISI VAE reconstruction-audit pre-flight (03) to Picasso.
#
# Usage (login node):
#   bash routines/preflights/maisi_vae/slurm/launcher_maisi_vae.sh
#   bash routines/preflights/maisi_vae/slurm/launcher_maisi_vae.sh path/to/other_config.yaml
#   bash routines/preflights/maisi_vae/slurm/launcher_maisi_vae.sh --dry-run
#
...
CONFIG_PATH="${CONFIG_PATH:-${REPO_DIR}/routines/preflights/maisi_vae/configs/picasso.yaml}"
...
JOB_NAME="brainrepa-preflight-maisi-vae-$(date -u +%Y%m%dT%H%M%SZ)"
...
SBATCH_CMD=(
    ...
    "${SCRIPT_DIR}/worker_maisi_vae.sh"
)
