...
- [ ] Library calls (MONAI, einops, torch.nn.functional, torchmetrics) preferred over manual implementations
...
- [ ] If a new library is introduced, declared in `pyproject.toml` `[project.dependencies]` and installed in the `brainrepa` env in the same change
...
5. Run: `~/.conda/envs/brainrepa/bin/python -m py_compile <file>` to verify syntax
6. Run: `~/.conda/envs/brainrepa/bin/python -m pytest tests/ -x -q` if relevant tests exist
7. Run: `~/.conda/envs/brainrepa/bin/python -m ruff check <file>` and `ruff format <file>`
