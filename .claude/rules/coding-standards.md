...
Project conda env: `brainrepa`. All Python invocations use `~/.conda/envs/brainrepa/bin/python`.
...
6. **Libraries-first**. Prefer well-maintained external libraries (MONAI, einops, torchmetrics, nibabel, SimpleITK, lpips, piq, h5py, hydra-core) over hand-rolled implementations — they are tested, peer-reviewed, and designed for the task. When adding a new dependency:
   - (a) declare it in `pyproject.toml` `[project.dependencies]` with a version pin,
   - (b) `~/.conda/envs/brainrepa/bin/pip install -e .` (or `pip install <pkg>`) in the same change,
...
7. **3D throughout the core pipeline.** MAISI VAE, BSF-S, and the FM generator all operate on 3D tensors. No 2D-slice operations except in clearly-labelled evaluation utilities (e.g. 2.5D LPIPS aggregation).
8. **Frozen pretrained models are immutable.** Never edit code under `src/external/` and never write to checkpoint paths declared in `src/external/README.md`. Adapter wrappers (e.g. T1-only BSF-S patch-embed rewrite) live in `src/brainrepa_fm/adapters/`. See `external-deps.md`.
9. **Tests use pytest** under the `brainrepa` env:
   ```
   ~/.conda/envs/brainrepa/bin/python -m pytest tests/ -v
   ```
   Markers: `unit`, `preflight_maisi`, `preflight_bsf`, `preflight_aug`, `fm`, `repa`, `gpu`, `slow`.
...
12. **Custom exceptions per module** (e.g. `class PreflightError(Exception): ...`, `class H5SchemaError(Exception): ...`). Library code never raises bare `Exception`.
...
14. **One class per file** when the class exceeds ~100 lines. Submodule organization under a namespace package.
